...
def main() -> int:
    ...
    ## construct a mapping via string hashing over the spatial ##
    add_latlong_strings(df)
    lat_long_loc_map = build_lat_long_loc_map(df)

    # print(json.dumps(lat_long_loc_map, indent=4))
    tmp: dict = {k: len(set(v)) for k, v in lat_long_loc_map.items()}
    print({k for k, v in lat_long_loc_map.items() if len(v) == 0})
    print("Max locations for a lat/long:", max(tmp.values()))
    print(
        "Lat/longs with multiple locations:",
        json.dumps(
            {k: v for k, v in lat_long_loc_map.items() if len(v) > 1},
            indent=4,
        ),
    )
